...
class AE_CIFAR10_V7(nn.Module):
    def __init__(self):
        super(AE_CIFAR10_V7, self).__init__()
    ...
    def forward(self, x):
        encoded = self.encoder_fc(self.encoder(x))
        decoded = torchvision.transforms.functional.crop(self.decoder(self.decoder_fc(encoded)), 0, 0, 32, 32)
        return encoded, decoded

    def td_encoder(self, x):
        return self.encoder_fc(self.encoder(x).detach())
