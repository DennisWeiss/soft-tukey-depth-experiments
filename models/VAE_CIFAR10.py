...
class VAE_CIFAR10(nn.Module):
    def __init__(self):
        ...
        self.encoder_fc_mu = nn.Linear(96 * 4 * 4, 512)
        self.encoder_fc_std = nn.Linear(96 * 4 * 4, 512)
    ...
    def forward(self, x):
        ...
        mu = self.encoder_fc_mu(encoded)
        std = self.encoder_fc_std(encoded)
        z = mu + torch.exp(std) * torch.randn(std.size(), device=std.get_device())
        decoded = torchvision.transforms.functional.crop(self.decoder(z), 0, 0, 32, 32)
        return mu, std, z, decoded