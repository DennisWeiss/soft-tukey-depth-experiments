...
class BasicBlockDecoder(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.deconv1 = nn.ConvTranspose2d(
            self.expansion*planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.deconv2 = nn.ConvTranspose2d(planes, planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.ConvTranspose2d(self.expansion*planes, planes,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes)
            )

    def forward(self, x):
        out = F.relu(self.bn1(self.deconv1(x)))
...
class BottleneckDecoder(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.deconv1 = nn.ConvTranspose2d(in_planes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.deconv2 = nn.ConvTranspose2d(planes, planes, kernel_size=3,
                               stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.deconv3 = nn.ConvTranspose2d(planes, planes, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(self.expansion*planes)
...
class ResNetDecoder(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.linear = nn.Linear(512, 512*block.expansion)
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        layers = []
        for stride in strides:
            layers.append(block(self.out_planes, planes, stride))
