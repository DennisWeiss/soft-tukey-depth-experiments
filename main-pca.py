...
from DataLoader import NominalMNISTDataset, AnomalousMNISTDataset, NominalCIFAR10Dataset, AnomalousCIFAR10Dataset, \
    NominalCIFAR10PCADataset, AnomalousCIFAR10PCADataset, ToyDataset


USE_CUDA_IF_AVAILABLE = False
DATASET_NAME = 'CIFAR10PCA'
...
for i in range(0, 1):
    train_data = NominalCIFAR10PCADataset(nominal_class=0, n_components=20, train=True)
    test_data = NominalCIFAR10PCADataset(nominal_class=i, n_components=20, train=False)
    ...
    def soft_tukey_depth(x, x_, z):
        return torch.sum(torch.sigmoid(torch.divide(torch.matmul(torch.subtract(x_, torch.matmul(torch.ones((x_.size(dim=0), 1), device=device), x)), z), torch.norm(z))))
    ...
    for item, x in enumerate(test_dataloader):
        ...
        optimizer = torch.optim.SGD([z], lr=1e-8)
