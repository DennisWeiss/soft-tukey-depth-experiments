...
for nominal_class in range(0, 10):
    ...
    data_latent = torch.zeros(len(data), 1, 32)

    dataloader = torch.utils.data.DataLoader(data)
    autoencoder = AE_CIFAR10_32().to(device)
    autoencoder.load_state_dict(torch.load(f'./snapshots/AE_CIFAR10_32_{nominal_class}'))
    autoencoder.eval()
    ...
    torch.save(data_latent, f"./datasets/CIFAR10_AE_representation/AE_CIFAR10_32_{'train' if TRAIN else 'test'}_{nominal_class}")