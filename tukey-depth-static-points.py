...
def draw_histogram(values, bins, title, temp, d):
    fig = plt.figure()
...
X = torch.normal(mean=0, std=1, size=(100, 2), device=device, requires_grad=False)
z0 = np.arange(-1, 1, 1e-2)
z1 = np.arange(-1, 1, 1e-2)
y = np.zeros((z0.shape[0], z1.shape[0]))

for i in range(z0.shape[0]):
    for j in range(z1.shape[0]):
        y[i, j] = soft_tukey_depth(X[0].reshape(1, -1), X, torch.tensor([[z0[i]], [z1[j]]], device=device, dtype=torch.float), 2).item()

ax = plt.axes()
ax.contourf(z0, z1, y, levels=1000)
plt.show()
...
#
#         draw_histogram(soft_tukey_depths, 20, f'temp={temp}, d={d}', temp, d)
